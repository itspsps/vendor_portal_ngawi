...
<div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        ...
        <div class="modal fade" id="modal_qc_bongkar_pk" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <form id="formantrian_bongkar_pk" class="m-form m-form--fit m-form--label-align-right" method="post" action="{{route('qc.lab.update_qc_bongkar')}}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('POST') }}
                        <div class="modal-header">
...
<script>
    $(function() {
        var table = $('#data_longgrain').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('qc.lab.antrian_qc_longgrain_index') }}",
            columns: [{
        ...
        //     ],
        //     "iDisplayLength": 10,
        //     ajax: "{{ route('qc.lab.antrian_qc_ciherang_index') }}",
        //     columns: [{
        //             data: "id_admin_timbangan",
        ...
        var table2 = $('#data_pw').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('qc.lab.antrian_qc_pandan_wangi_index') }}",
            columns: [{
                    data: "id_admin_timbangan",
        ...
        var table3 = $('#data_kp').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('qc.lab.antrian_qc_ketan_putih_index') }}",
            columns: [{
                    data: "id_admin_timbangan",
...
<script>
    $(function() {
        var table = $('#datatable2').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('qc.lab.antrian_qc_bongkar_pk_index') }}",
            columns: [{
                    data: "id_admin_timbangan",
...
<script type="text/javascript">
    $(function() {
                        ...
                        Swal.fire({
                            ...
                            onBeforeOpen: () => {
                                ...
                                $.ajax({
                                    ...
                                    url: "{{route('qc.lab.update_qc_bongkar')}}",
                                    type: "POST",
                                    dataType: 'json',
        ...
        $(document).on('click', '.to_qc_bongkar_gb', function() {
            ...
            var url = "{{ route('qc.lab.show_qc_bongkar_gb_show') }}" + "/" + id;
            // console.log(url);
        ...
        $(document).on('click', '.to_qc_bongkar_pk', function() {
            var id = $(this).attr("name");
            var url = "{{ route('qc.lab.show_qc_bongkar_pk_show') }}" + "/" + id;
            console.log(id);
            $.ajax({
                type: "GET",
                url: url,
