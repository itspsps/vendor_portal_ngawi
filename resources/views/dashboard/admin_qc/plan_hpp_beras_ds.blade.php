...
<script>
    ...
    var table = $('#table-plan-hpp').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('qc.lab.plan_hpp_beras_ds_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    $(document).ready(function() {
        $('body').on('click', '#btnEdit', function() {
            ...
            var url = "{{ route('qc.lab.partial_plan_hpp_beras_ds', ':id') }}";
    ...
    $('body').on('click', '#btnDelete', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('qc.lab.delete_plan_hpp_beras_ds') }}/" + cek,
                    type: "GET",
                    error: function() {
                        Swal.fire({
                            title: 'Gagal!',
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        url: "{{ route('qc.lab.simpan_plan_hpp_beras_ds') }}",
                        type: "POST",
                        dataType: 'json',
                        success: function(data) {
