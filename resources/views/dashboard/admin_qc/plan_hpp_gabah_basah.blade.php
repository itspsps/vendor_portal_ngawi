...
<script>
    ...
    var table = $('#data_longgrain').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('qc.lab.plan_hpp_gabah_basah_longgrain_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    var table2 = $('#data_pw').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('qc.lab.plan_hpp_gabah_basah_pandanwangi_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    var table4 = $('#data_kp').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('qc.lab.plan_hpp_gabah_basah_ketanputih_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
            ...
            if (result.value) {
                Swal.fire({
                    ...
                    onBeforeOpen: () => {
                        Swal.showLoading()
                        $.ajax({
                            url: "{{ route('qc.lab.delete_plan_hpp_gabah_basah') }}/" + cek,
                            type: "GET",
                            error: function() {
                                Swal.fire({
                                    title: 'Gagal!',
                        ...
                        Swal.fire({
                            ...
                            onBeforeOpen: () => {
                                ...
                                $.ajax({
                                    ...
                                    url: "{{ route('qc.lab.simpan_plan_hpp_gabah_basah') }}",
                                    type: "POST",
                                    dataType: 'json',
                                    success: function(data) {
