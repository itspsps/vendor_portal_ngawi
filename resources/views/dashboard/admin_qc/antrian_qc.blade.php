...
<div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        ...
        <div class="modal fade" id="modal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <form id="formantrian_qc" class="m-form m-form--fit m-form--label-align-right" method="post" action="{{ route('qc.lab.save_gabah_incoming_qc') }}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('POST') }}
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            ...
                            <input type="hidden" id="penerimaan_id_data_po_gb" name="gabahincoming_id_data_po_gb">
                            <input type="hidden" id="id_penerimaan_po_gb" name="id_penerimaan_po_gb">
                            <input type="hidden" id="penerimaan_id_bid_user_gb" name="gabahincoming_id_bid_user_gb">
                            ...
                            <div class="m-form__group form-group" id="ifYes" style="display: none;">
                                ...
                                <div class="kt-radio-inline">
                                    <label class="kt-radio">
                                        <input type="radio" value="UTARA" id="lokasigt" class="form-control m-input" name="lokasi_gt"> UTARA
                                        <span></span>
                                    </label>
                                    <label class="kt-radio">
                                        <input type="radio" value="SELATAN" id="lokasi04" class="form-control m-input" name="lokasi_gt"> SELATAN
                                        <span></span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="m-form__group form-group">
                                ...
                                <input readonly type="number" step="any" required name="plan_harga_gb" id="plan_harga_gb" class="form-control m-input">
                            </div>
...
<script>
    $(function() {
        var table = $('#datatable').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('qc.lab.antrian_qc_index') }}",
            columns: [{
                    data: "id_bid",
    ...
    $(function() {
        $(document).on('click', '#btn_save', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    var lok = document.getElementById('lokasigt');
                    var lok1 = document.getElementById('lokasi04');
                    if ($('#kadar_air_gb').val() == '' | $('#ka_kg_gb').val() == '' | lok.checked == false && lok1.checked == false | $('#berat_sample_awal_ks_gb').val() == '' | $('#berat_sample_awal_kg_gb').val() == '' | $('#berat_sample_akhir_kg_gb').val() == '' | $('#berat_sample_pk_gb').val() == '' | $('#randoman_gb').val() == '' | $('#wh_gb').val() == '' | $('#tp').val() == '' | $('#md_gb').val() == '' | $('#keterangan_lab_1').find(":selected").val() == '' | $('#broken_gb').val() == '' | $('#keterangan_lab1').val() == '') {
        ...
        $(document).on('click', '.proses_lab_1', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#penerimaan_id_data_po').val(parsed.penerimaan_id_data_po);
                    $('#penerimaan_id_bid_user').val(parsed.penerimaan_id_bid_user);
                    ...
                    $('#min_tp').val(parsed.min_tp);
                    $('#max_tp').val(parsed.max_tp);
                    $('#harga_hpp').val(parsed.harga);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(parsed, function(item) {
                        my_orders.append("<input type=" + 'text' + " class=" + 'hpp' + " value=" + parsed[item].min_tp_gb + '#' + parsed[item].max_tp_gb + '#' + parsed[item].harga_gb + ">");
                    });
                }
            });
        });
        ...
        $(document).on('click', '.lokasi_bongkar', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // console.log(response);
                    var parsed = $.parseJSON(response);
                    $('#lokasi_bongkar').text("Gudang = " + parsed.lokasi_bongkar);
                    $('#antrian_bongkar').text("Antrian = " + parsed.antrian);
                }
            });
        });
    });
...
<script>
    ...
    function rumus() {
        ...
        if (kadar_air_gb.value == 0 || kadar_air_gb.value == '' ||
            ...
            $.ajax({
                type: "GET",
                url: "{{route('get_price_top')}}" + "/" + id_penerimaan,
                async: false,
            ...
            $.ajax({
                type: "GET",
                url: "{{route('get_count_plan_hpp')}}" + "/" + id_penerimaan,
                async: false,
                success: function(data) {
                    var record = JSON.parse(data);
                    console.log(record)
                    if (record == '0') {
                        Swal.fire({
                            ...
                            if (result.value) {
                                window.location.href = "{{ route('qc.lab.plan_hpp') }}";
