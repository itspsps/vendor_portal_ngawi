    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <div class="col-xl-12 col-lg-12 col-md-12 order-lg-1 order-xl-1">
            <div class="kt-portlet kt-portlet--mobile">
                ...
                <form class="" method="post" action="{{route('master.lab.download_output_lab1_excel')}}" enctype="multipart/form-data">
...
<div class="modal fade" id="modal_to_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <form id="form_edit" class="m-form m-form--fit m-form--label-align-right" method="post" action="{{ route('master.lab.update_gabah_incoming_qc') }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('POST') }}
    ...
    $(document).ready(function() {
        ...
        function load_data(from_date = '', to_date = '') {
            var table = $('#datatable').DataTable({
                ...
                "iDisplayLength": 10,
                ajax: {
                    url: "{{ route('master.lab.output_lab1_gb_index') }}",
                },
                columns: [{
                        data: "id_bid",
...
<script type="text/javascript">
    $(function() {
        $(document).on('click', '#to_edit', function() {
            ...
            var url = "{{ route('master.lab.edit_lab1') }}" + "/" + id;
            var url2 = "{{route('master.lab.get_plan_hpp') }}" + "/" + tanggal_po;
        ...
        $(document).on('click', '#btn_bongkar', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "GET",
                        url: "{{route('master.lab.approve_lab1') }}/" + id,
                        error: function() {
