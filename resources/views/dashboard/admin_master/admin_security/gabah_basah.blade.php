        ...
        <div class="modal-content">
            <form id="form_terimadatapo" class="m-form m-form--fit m-form--label-align-right" method="post" action="javascript:void(0);" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <br>
                    <span class="btn btn-label-primary text-center" style="text-align: center;" id="jam_sekarang"></span>
...
<script>
    ...
    $(function() {
        var table1 = $('#po_kemarin').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('master.security.gabahbasah_index_kemarin') }}",
            columns: [{
                    data: "id_bid",
        ...
        var table2 = $('#datatable_sekarang').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('master.security.gabahbasah_index_sekarang') }}",
            columns: [{
                    data: "id_bid",
...
<script type="text/javascript">
    $(function() {
        $(document).on('click', '.toedit', function() {
            ...
            var url = "{{ route('master.security.show.penerimaan_po') }}" + "/" + id;
            console.log(url);
            $.ajax({
                type: "GET",
                url: url,
                success: function(response) {
                    ...
                    function customClock() {
                        var time = new Date()
                        var date = time.getDate() + '-' + (time.getMonth() + 1).toString().padStart(2, "0") + '-' + time.getFullYear();
                        ...
                        $('#jam_sekarang').html('<b>' + date + '&nbsp;' + hrs + ":" + min + ":" + sec + '</b>');
                    }
                }
            });
        });
                        ...
                        Swal.fire({
                            ...
                            onBeforeOpen: () => {
                                ...
                                $.ajax({
                                    ...
                                    url: "{{route('master.security.terima_data_po')}}",
                                    type: "POST",
                                    dataType: 'json',
