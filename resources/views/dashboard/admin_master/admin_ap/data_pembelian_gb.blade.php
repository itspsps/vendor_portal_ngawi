...
<script>
    $(function() {
        var table = $('#data_verifikasi').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('master.data_pembelian_gb_longgrain_index') }}",
            columns: [{
        ...
        var table2 = $('#data_verified').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('master.data_pembelian_gb_longgrain1_index') }}",
            columns: [{
                    data: "id_penerimaan_po",
...
<script>
    function getcount_verifikasi() {
        $.ajax({
            type: "GET",
            url: "{{route('master.getcount_verifikasi')}}",
            success: function(data) {
                $("#count_verifikasi").empty();
                var notif = JSON.parse(data);
                $("#count_verifikasi").html(notif);
            }
        });
    }

    function getcount_verified() {
        $.ajax({
            type: "GET",
            url: "{{route('master.getcount_verified')}}",
            success: function(data) {
                $("#count_verified").empty();
                var notif = JSON.parse(data);
                $("#count_verified").html(notif);
            }
        });
    }
    setInterval(getcount_verified, 2000);
