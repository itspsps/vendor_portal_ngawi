...
<script>
    ...
    var table = $('#table-plan-hpp').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('master.plan_hpp_pecah_kulit_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    $(document).ready(function() {
        $('body').on('click', '#btnEdit', function() {
            ...
            var url = "{{ route('master.partial_plan_hpp_pecah_kulit', ':id') }}";
    ...
    $('body').on('click', '#btnDelete', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('master.delete_plan_hpp_pecah_kulit') }}/" + cek,
                    type: "GET",
                    error: function() {
                        Swal.fire({
                            title: 'Gagal!',
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        url: "{{ route('master.simpan_plan_hpp_pecah_kulit') }}",
                        type: "POST",
                        dataType: 'json',
                        success: function(data) {
