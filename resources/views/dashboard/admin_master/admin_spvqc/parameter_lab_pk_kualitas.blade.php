                    ...
                    <div class="accordion  accordion-toggle-arrow" id="accordionExample4">
                        <div class="card">
                            ...
                            <div id="collapseOne4" class="collapse" aria-labelledby="headingOne" data-parent="#accordionExample4">
                                <div class="card-body">
                                    <form class="kt-form" id="form_lab_kualitas" action="{{ route('master.parameter_lab_pk_kualitas_store') }}" method="post" enctype="multipart/form-data">
                                        @csrf
            ...
            <div class="modal fade" id="modal_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <form class="m-form m-form--fit m-form--label-align-right" method="post" action="{{ route('master.parameter_lab_pk_kualitas_update') }}" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            {{ method_field('POST') }}
...
<script>
    $(function() {
        var table = $('#table-parameter-lab').DataTable({
            ...
            "iDisplayLength": 10,
            ajax: "{{ route('master.parameter_lab_pk_kualitas_index') }}",
            columns: [{
                    data: "id_bid",
...
<script type="text/javascript">
    $(function() {
        $(document).on('click', '.to_parameter', function() {
            ...
            var url = "{{ route('master.parameter_lab_pk_kualitas_show') }}" + "/" + id;
            console.log(url);
            $.ajax({
                type: "GET",
                url: url,
    ...
    $(document).ready(function() {
        ...
        $('body').on('click', '#btn_delete_kualitas', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{route('master.parameter_lab_pk_kualitas_destroy')}}/" + id,
                        type: "GET",
                        error: function() {
                            Swal.fire({
                                title: 'Gagal!',
