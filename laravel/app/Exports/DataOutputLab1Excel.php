...
class DataOutputLab1Excel implements FromCollection, WithHeadings, WithEvents, WithTitle, WithCustomStartCell
{
    /**
    ...
    public function collection()
    {
        if (($this->from_date && $this->to_date) == '') {
            return DB::table('data_po')->join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
                ...
                ->select('lokasi_bongkar_gb', 'tanggal_po', 'jam_masuk', 'lab1_plat_gb', 'nama_vendor', 'keterangan_penerimaan_po', 'kadar_air_gb', 'ka_kg_gb', 'berat_sample_awal_ks_gb', 'berat_sample_awal_kg_gb', 'berat_sample_akhir_kg_gb', 'berat_sample_pk_gb', 'randoman_gb', 'wh_gb', 'tp_gb', 'md_gb', 'broken_gb', 'hampa_gb', 'kg_after_adjust_hampa_gb', 'prosentasi_kg_gb', 'susut_gb', 'adjust_susut_gb', 'prsentase_ks_kg_after_adjust_susut_gb', 'prsentase_kg_pk_gb', 'adjust_prosentase_kg_pk_gb', 'presentase_ks_pk_gb', 'presentase_putih_gb', 'adjust_prosentase_kg_ke_putih_gb', 'plan_rend_dari_ks_beras_gb', 'katul_gb', 'refraksi_broken_gb', 'plan_harga_gb', 'plan_harga_gabah_gb')
                ->get();
        } else {
            return DB::table('data_po')->join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
                ->join('users', 'users.id', '=', 'data_po.user_idbid')
            ...
            AfterSheet::class    => function (AfterSheet $event) {
                if (($this->from_date && $this->to_date) == '') {
                    $query = DB::table('data_po')->join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
                        ...
                        ->where('penerimaan_po.status_penerimaan', '>=', 5)
                        ->orderBy('penerimaan_po.id_penerimaan_po', 'ASC')
                        ->select('lokasi_bongkar_gb', 'tanggal_po', 'jam_masuk', 'lab1_plat_gb', 'nama_vendor', 'keterangan_penerimaan_po', 'kadar_air_gb', 'ka_kg_gb', 'berat_sample_awal_ks_gb', 'berat_sample_awal_kg_gb', 'berat_sample_akhir_kg_gb', 'berat_sample_pk_gb', 'randoman_gb', 'wh_gb', 'tp_gb', 'md_gb', 'broken_gb', 'hampa_gb', 'kg_after_adjust_hampa_gb', 'prosentasi_kg_gb', 'susut_gb', 'adjust_susut_gb', 'prsentase_ks_kg_after_adjust_susut_gb', 'prsentase_kg_pk_gb', 'adjust_prosentase_kg_pk_gb', 'presentase_ks_pk_gb', 'presentase_putih_gb', 'adjust_prosentase_kg_ke_putih_gb', 'plan_rend_dari_ks_beras_gb', 'katul_gb', 'refraksi_broken_gb', 'plan_harga_gabah_gb', 'plan_harga_gabah_gb')
                        ->count();
                } else {
                    $query = DB::table('data_po')->join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
                        ->join('users', 'users.id', '=', 'data_po.user_idbid')
                ...
                $event->sheet->setCellValue('P1', 'LAPORAN KUALITAS PEMBELIAN GABAH PT. SURYA PANGAN SEMESTA NGAWI');
                // $event->sheet->setCellValue('T_gb','SECURITY CLASSIFICATION - UNCLASSIFIED [Generator: Admin]');
