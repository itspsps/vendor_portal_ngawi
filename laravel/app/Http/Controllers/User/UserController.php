...
class UserController extends Controller
{
    ...
    public function data_list_po($id)
    {
        $data = DataPO::join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
            ...
            ->selectRaw(
                'data_po.*,users.nama_vendor,penerimaan_po.*,lab2_gb.aksi_harga_gb,lab2_gb.harga_akhir_gb,bid.name_bid,bid.open_po,bid_user.id_biduser,bid_user.jumlah_kirim'
            )
    ...
    public function new_home()
    {
        ...
        $site_other = DB::table('bid')
            ...
            ->limit(5)
            ->get();
        $site_subang_ciherang = DB::table('bid')
            ->where('lokasi', 'SUBANG')
            ->where('name_bid', 'GABAH BASAH CIHERANG')
            ->orderBy('date_bid', 'DESC')
            ->limit(5)
            ->get();
        $site_subang_pandanwangi = DB::table('bid')
            ->where('lokasi', 'SUBANG')
            ->where('name_bid', 'GABAH BASAH PANDAN WANGI')
            ->orderBy('date_bid', 'DESC')
            ->limit(5)
            ->get();
        $site_subang_ketanputih = DB::table('bid')
            ->where('lokasi', 'SUBANG')
            ->where('name_bid', 'GABAH BASAH KETAN PUTIH')
            ->orderBy('date_bid', 'DESC')
            ->limit(5)
        ...
        $lainnya = Bid::orderBy('id_bid', 'desc')
            ...
            ->get();
        $berita = news::get();
        $get_site_subang_longgrain = DB::table('bid')
            ->where('lokasi', 'SUBANG')
            ->where('name_bid', 'GABAH BASAH LONG GRAIN')
            ->where('bid_status', '1')
            ->orderBy('id_bid', 'ASC')
            ->first();
        if ($get_site_subang_longgrain == '') {
            $site_subang_longgrain = DB::table('bid')
                ->where('lokasi', 'SUBANG')
                ->where('name_bid', 'GABAH BASAH LONG GRAIN')
                ->where('bid_status', '1')
                ->orderBy('id_bid', 'DESC')
            ...
            if ($date <= $get_site_subang_longgrain->batas_bid) {
                $site_subang_longgrain = DB::table('bid')
                    ->where('lokasi', 'SUBANG')
                    ->where('name_bid', 'GABAH BASAH LONG GRAIN')
                    ->where('bid_status', '1')
        ...
        $notif = DB::table('broadcast')->count();
        // dd($site_SUBANG_pandanwangi);
        return view('dashboard.user.new_user.home', [
            'site_kediri' => $site_kediri,
            'site_subang' => $site_subang,
            'notif' => $notif,
            'count_transaksi' => $count_transaksi,
            'site_subang_longgrain' => $site_subang_longgrain,
            'site_subang_ciherang' => $site_subang_ciherang,
            'site_subang_pandanwangi' => $site_subang_pandanwangi,
            'site_subang_ketanputih' => $site_subang_ketanputih,
            'site_other' => $site_other,
            'lainnya' => $lainnya,
            'berita' => $berita,
        ]);
