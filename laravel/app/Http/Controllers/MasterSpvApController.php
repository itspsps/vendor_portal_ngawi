...
class MasterSpvApController extends Controller
{
    public function approve_receipt($id)
    {
        ...
        $url = 'http://epicor.suryapangansemesta.com:2022/api/PO/UpdatePO';
        $form_params = [
            'PONum'         => $get->PONum,
            'Quantity'      => $get->netto2,
            'UnitPrice'     => $get_harga,
    ...
    public function kirim_epicor_gb($id)
    {
        ...
        $data->update();
        $client = new \GuzzleHttp\Client();
        $url = 'http://epicor.suryapangansemesta.com:2022/api/PO/ApprovalPO?PONum=' . $data->penerimaan_po_num;
    ...
    public function kirim_epicor_pk($id)
    {
        ...
        $client = new \GuzzleHttp\Client();
        $url = 'http://epicor.suryapangansemesta.com:2022/api/PO/ApprovalPO?PONum=' . $get_id->penerimaan_po_num;
        $response = $client->get($url);
        $response = $response->getBody()->getContents();
        // dd($response); 
        // return json_encode($update_status_penerimaan_po);
