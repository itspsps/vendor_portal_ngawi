...
<script>
    ...
    var table1 = $('#data_longgrain').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('master.plan_hpp_gabah_basah_longgrain_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    var table2 = $('#data_pw').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('master.plan_hpp_gabah_basah_pandanwangi_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    var table3 = $('#data_kp').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('master.plan_hpp_gabah_basah_ketanputih_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
            ...
            if (result.value) {
                Swal.fire({
                    ...
                    onBeforeOpen: () => {
                        Swal.showLoading()
                        $.ajax({
                            url: "{{ route('master.delete_plan_hpp_gabah_basah') }}/" + cek,
                            type: "GET",
                            error: function() {
                                Swal.fire({
                                    title: 'Gagal!',
                        ...
                        Swal.fire({
                            ...
                            onBeforeOpen: () => {
                                ...
                                $.ajax({
                                    ...
                                    url: "{{ route('master.simpan_plan_hpp_gabah_basah') }}",
                                    type: "POST",
                                    dataType: 'json',
                                    success: function(data) {
