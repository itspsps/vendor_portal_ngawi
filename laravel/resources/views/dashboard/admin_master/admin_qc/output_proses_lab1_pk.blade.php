    ...
    <div class="modal fade" id="modal_to_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form id="form_edit" class="m-form m-form--fit m-form--label-align-right" method="post" action="{{ route('master.update_proseslab1_pecah_kulit') }}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    ...
                    <div class="modal-body">
                        ...
                        <input type="text" id="presentase_butir_patah_beras_adjust_pk" name="presentase_butir_patah_beras_adjust_pk">
                        ...
                        <input type="text" id="harga_incoming_pk" name="harga_incoming_pk">
                        <input type="text" id="plan_harga_aktual_pk" name="plan_harga_aktual_pk">
                        <input type="text" id="aktual_kualitas_pk" name="aktual_kualitas_pk">
                        <input type="text" id="harga_awal_pk" name="harga_awal_pk">
                        <input type="text" id="aksi_harga_pk" name="aksi_harga_pk">
                        <input type="text" id="reaksi_harga_pk" name="reaksi_harga_pk">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#datatable_pk').DataTable({
                ...
                "iDisplayLength": 10,
                ajax: {
                    url: "{{ route('master.output_lab1_qc_pk_index') }}",
                },
                "aoColumnDefs": [{
                    "bVisible": false,
                    "aTargets": [2]
    ...
    <script type="text/javascript">
        $(function() {
            $(document).on('click', '#to_edit', function() {
                ...
                var url = "{{ route('master.edit_lab1_pk') }}" + "/" + id;
                    ...
                    success: function(response) {
                        ...
                        if (parsed == '' | parsed == null) {
                            Swal.fire({
                                ...
                                if (result.value) {
                                    window.location.href = "{{ route('master.harga_atas_pecah_kulit') }}";
            ...
            $(document).on('click', '#btn_bongkar', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "GET",
                            url: "{{route('master.approve_lab1_pk') }}/" + id,
                            error: function() {
                    ...
                    if (result.value) {
                        Swal.fire({
                            ...
                            onBeforeOpen: () => {
                                ...
                                $.ajax({
                                    type: "GET",
                                    url: "{{route('master.approve_tolak_lab1_pk') }}/" + id,
                                    error: function() {
                                        alert('Something is wrong');
                                    },
                                    success: function(data) {
