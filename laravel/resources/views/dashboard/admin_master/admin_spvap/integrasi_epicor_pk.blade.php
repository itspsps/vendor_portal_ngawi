    ...
    $(document).ready(function() {
        ...
        function load_data(from_date = '', to_date = '') {
            var table = $('#datatable').DataTable({
                ...
                "iDisplayLength": 10,
                ajax: {
                    url: "{{route('master.integrasi_epicor_pk_index')}}",
                    data: {
                        from_date: from_date,
                        to_date: to_date
                    }
            ...
            var table1 = $('#datatable1').DataTable({
                ...
                "iDisplayLength": 10,
                ajax: {
                    url: "{{route('master.integrasi_epicor_pk1_index')}}",
                    data: {
                        from_date: from_date,
                        to_date: to_date
                    }
                    ...
                    if (result.value) {
                        Swal.fire({
                            ...
                            onBeforeOpen: () => {
                                Swal.showLoading()
                                $.ajax({
                                    url: "{{ route('master.kirim_epicor_pk_all')}}",
                                    method: "get",
                                    data: {
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        ...
                        onBeforeOpen: () => {
                            Swal.showLoading()
                            $.ajax({
                                url: "{{route('master.approve_receipt_pk')}}/" + cek,
                                type: "GET",
                                error: function() {
                                    Swal.fire({
                                        title: 'Gagal!',
