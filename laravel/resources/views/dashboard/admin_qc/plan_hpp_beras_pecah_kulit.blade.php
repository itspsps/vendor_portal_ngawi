        ...
        <div class="col-xl-12 col-lg-12 col-md-12 order-lg-1 order-xl-1">
            <div class="kt-portlet kt-portlet--mobile">
                ...
                <div class="kt-portlet__body">
                    ...
                    @elseif(Auth::guard('lab')->user()->level=='MANAGER')
                    <table class="table table-bordered" id="table-plan-hpp1">
                        <thead>
                            <tr>
                                <th style="text-align: center;width:2%">No</th>
                                <th style="text-align: center;width:auto">MIN&nbsp;TP</th>
...
<script>
    ...
    var table = $('#table-plan-hpp').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('qc.lab.plan_hpp_pecah_kulit_index') }}",
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
            ...
            {
                data: 'waktu_plan_hpp_pk',
                name: 'waktu_plan_hpp_pk'
            },
            {
                data: 'harga_pk',
                name: 'harga_pk'
    ...
    var table = $('#table-plan-hpp1').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('qc.lab.plan_hpp_pecah_kulit_index') }}",
        columns: [{
    ...
    $(document).ready(function() {
        $('body').on('click', '#btnEdit', function() {
            ...
            var url = "{{ route('qc.lab.partial_plan_hpp_pecah_kulit', ':id') }}";
            url = url.replace(':id', ids);
            $.get(url, function(response) {
                $('#id_plan_hpp').val(response.id_plan_hpp_pk);
                $('#min_tp').val(response.min_tp_pk);
                $('#max_tp').val(response.max_tp_pk);
                $('#harga').val(response.harga_pk);
                $('#tanggal_po').val(response.waktu_plan_hpp_pk);
                $('#tanggal_poedit').val(response.waktu_plan_hpp_pk);
    ...
    $('body').on('click', '#btnDelete', function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{ route('qc.lab.delete_plan_hpp_pecah_kulit') }}/" + cek,
                    type: "GET",
                    error: function() {
                        Swal.fire({
                            title: 'Gagal!',
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        url: "{{ route('qc.lab.simpan_plan_hpp_pecah_kulit') }}",
                        type: "POST",
                        dataType: 'json',
                        success: function(data) {
