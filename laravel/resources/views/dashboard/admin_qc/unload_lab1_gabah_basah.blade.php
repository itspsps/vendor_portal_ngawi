                ...
                <div class="kt-portlet__head kt-portlet__head--lg">
                    <div class="kt-portlet__head-label">
                        ...
                        <h3 class="kt-portlet__head-title">
                            Data Incoming (Bongkar) Gabah Basah
                        </h3>
                    </div>
                </div>
...
<div class="modal fade" id="excel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form class="m-form m-form--fit m-form--label-align-right" method="post" action="{{route('qc.lab.download_data_unload_excel')}}" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('POST') }}
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Download Berdasarkan Tanggal</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="kt-scroll" data-scroll="true" data-height="200" style="height: 200px; overflow: auto;">
                        <input type="text" name="daterange" class="form-control m-input" />
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger m-btn" data-dismiss="modal">Close</button>
    ...
    $(document).ready(function() {
        ...
        function load_data(from_date = '', to_date = '') {
            var table = $('#datatable').DataTable({
                ...
                "iDisplayLength": 10,
                ajax: {
                    url: "{{ route('qc.lab.unload_lab1_gabah_basah_index') }}",
                    data: {
                        from_date: from_date,
                        to_date: to_date
                    }
    ...
    $(function() {
        ...
        $('#btn_export').click(function() {
            ...
            $.ajax({
                ...
                url: "{{route('qc.lab.download_data_unload_excel')}}",
                type: "POST",
                cache: false,
                xhrFields: {
                    responseType: 'blob'
