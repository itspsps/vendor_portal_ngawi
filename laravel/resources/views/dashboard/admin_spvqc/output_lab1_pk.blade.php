    ...
    $(document).ready(function() {
        ...
        function load_data(from_date = '', to_date = '') {
            var table = $('#datatable1').DataTable({
                ...
                "iDisplayLength": 10,
                ajax: {
                    url: "{{ route('qc.spv.output_lab1_pk_index') }}",
                },
                "aoColumnDefs": [{
                    "bVisible": false,
                    "aTargets": [2]
        ...
        $(document).on('click', '#btn_bongkar', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "GET",
                        url: "{{route('qc.spv.approve_lab1_pk') }}/" + id,
                        error: function() {
        ...
        $(document).on('click', '#btn_tolakbongkar', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "GET",
                        url: "{{route('qc.spv.notapprove_lab1_pk') }}/" + id,
                        error: function() {
                            alert('Something is wrong');
                        },
                        success: function(data) {
                ...
                if (result.value) {
                    Swal.fire({
                        ...
                        onBeforeOpen: () => {
                            Swal.showLoading()
                            $.ajax({
                                url: "{{route('qc.spv.approve_tolak_lab1_pk') }}/" + id,
                                type: "GET",
                                error: function() {
                                    alert('Something is wrong');
                                },
