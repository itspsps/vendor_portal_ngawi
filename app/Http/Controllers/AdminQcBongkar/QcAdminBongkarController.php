...
class QcAdminBongkarController extends Controller
{
    ...
    {
        // dd(DataPO::join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
        //     ->join('users', 'users.id', '=', 'data_po.user_idbid')
        ...
        //     ->select('data_po.*', 'users.nama_vendor')
        //     ->get());
    ...
    public function update_dtm(Request $request)
    {
        $data = DataQcBongkar::where('kode_po_bongkar', $request->penerimaan_kode_po)
            ->update([
                ...
                'tempat_bongkar' => $request->lokasi_bongkar,
            ]);
        $data = Lab2GabahBasah::where('lab2_kode_po_gb', $request->penerimaan_kode_po)
            ->update([
                'dtm_gb' => $request->dtm_gb,
                'lokasi_bongkar_gb' => $request->lokasi_bongkar,
                'tempat_gb' => $request->lokasi_bongkar,
            ]);
        $lab1 = Lab1GabahBasah::where('lab1_kode_po_gb', $request->penerimaan_kode_po)
            ->update([
                'lokasi_bongkar_gb' => $request->lokasi_bongkar,
                'lokasi_gt_gb' => $request->lokasi_bongkar,
            ]);
        ...
        $log->aktivitas_qc                  = 'Revisi BKS DTM. Kode PO:' . $request->penerimaan_kode_po . '. NO DTM  : BKS.NGW-' . $request->dtm_gb . ' LOKASI : ' . $request->lokasi_bongkar;
        $log->keterangan_aktivitas          = 'Selesai';
        $log->created_at                    = date('Y-m-d H:i:s');
        $log->save();
        ...
        if ($po == NULL) {
            ...
            $po->proses_tracker  = 'REVISI UPDATE BKS / LOKASI';
