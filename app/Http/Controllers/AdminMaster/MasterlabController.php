...
class MasterlabController extends Controller
{
    ...
    public function proses_lab1_gabah_basah_longgrain_index()
    {
        ...
        return Datatables::of(DataPO::join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
            ...
            ->addColumn('ckelola', function ($list) {
                ...
                return
                    '<button style="margin:2px;" name="' . $list->id_penerimaan_po . '" data-vendor="' . $list->nama_vendor . '" data-tanggalpo="' . $list->tanggal_po . '"  data-item="' . $list->name_bid . '"  title="Proses Data" class="btn btn-outline-success m-btn m-btn--icon btn-sm m-btn--icon-only">
                        Lab&nbsp;Process
                    </button>';
            })
    ...
    public function proses_lab2_gabah_basah_longgrain_index()
    {
        ...
        return Datatables::of(DB::table('data_po')->join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
            ...
            ->addColumn('detail_hasil_qc', function ($list) {
                return
                    '
                <button style="margin:2px;" name="' . $list->id_lab1_gb . '" data-id="' . $list->id_penerimaan_po . '" data-tanggalpo="' . $list->tanggal_po . '"  data-item="' . $list->name_bid . '"  title="Information" class="detail_data btn btn-outline-info m-btn m-btn--icon btn-sm m-btn--icon-only">
                Lab&nbsp;Process
                </button>';
            })
            ->rawColumns(['kode_po', 'nama_vendor', 'tanggal_po', 'waktu_penerimaan', 'nama_penerima_po', 'plat_kendaraan', 'ckelola', 'detail_hasil_qc'])
    ...
    public function count_outputlab1_gb()
    {
        $count_success = DataPO::join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
            ...
            ->where('bid.name_bid', 'GABAH BASAH LONG GRAIN')
            ->count();
        $count_tolak = DataPO::join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
            ->join('penerimaan_po', 'penerimaan_po.penerimaan_id_data_po', '=', 'data_po.id_data_po')
            ->join('lab1_gb', 'lab1_gb.lab1_id_data_po_gb', '=', 'data_po.id_data_po')
            ->where('lab1_gb.status_approved', '=', 1)
            ->where('lab1_gb.status_lab1_gb', '=', 5)
