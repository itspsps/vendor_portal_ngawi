...
class QcAdminController extends Controller
{
    ...
    public function revisi_security($id)
    {
        ...
        if ($get_data_po->batas_penerimaan_po <= $date) {
            $data = PenerimaanPO::where('id_penerimaan_po', $id)->update([
                'analisa' => 'revisi',
                'penerimaan_po.id_adminanalisa' => 1,
                'penerimaan_po.status_analisa' => 2,
                'penerimaan_po.status_revisi' => 0,
                'keterangan_analisa' => 'Nopol Salah'
            ]);
        } else {
            $data = DataPO::where('id_data_po', $get_data_po->penerimaan_id_data_po)->update(['nopol' => NULL, 'status_bid' => '1']);
